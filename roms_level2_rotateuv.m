...
p.addParameter('ftype', {'average', 'history'}, @(x) ischar(x) || iscellstr(x));
...
for ii = 1:length(Opt.ftype)
    ufile = fullfile(Opt.lev1folder, sprintf('%s_%s_u.nc', Opt.outbase, Opt.ftype{ii}));
    vfile = fullfile(Opt.lev1folder, sprintf('%s_%s_v.nc', Opt.outbase, Opt.ftype{ii}));
    
    unew = fullfile(Opt.outfolder, sprintf('%s_%s_uEast.nc', Opt.outbase, Opt.ftype{ii}));
    vnew = fullfile(Opt.outfolder, sprintf('%s_%s_vNorth.nc', Opt.outbase, Opt.ftype{ii}));
    ...
    if exist(ufile,'file') && ~exist(unew,'file')
        cmd = sprintf('ncks -F -v ocean_time,s_rho -d ocean_time,1,1 %s %s', ufile, unew);
        system(cmd);
    ...
    if exist(vfile,'file') && ~exist(vnew,'file')
        cmd = sprintf('ncks -F -v ocean_time,s_rho -d ocean_time,1,1 %s %s', vfile, vnew);
        system(cmd);
...
%     if exist(ufile,'file') && exist(vfile, 'file') && ~exist(unew,'file') && ~exist(vnew, 'file')
%         cmd = sprintf('ncks -F -v ocean_time,s_rho -d ocean_time,1,1 %s %s', ufile, unew);
%         system(cmd);
%     
%         cmd = sprintf('ncks -A -v lat_rho,lon_rho %s %s', Opt.gridfile, unew);
...
%     end
    ...
    uunit  = ncreadatt(ufile, 'u', 'units');
    ulname = ncreadatt(ufile, 'u', 'long_name');
    vunit  = ncreadatt(vfile, 'v', 'units');
    vlname = ncreadatt(vfile, 'v', 'long_name');
    
    Iu = ncinfo(unew);
    if ~ismember('uEast', {Iu.Variables.Name})
        nccreate(unew, 'uEast', ...
            'Dimensions', {'xi_rho', ...
                           'eta_rho', ...
                           's_rho', ...
                           'ocean_time'}, ...
            'Datatype', 'single');
        ncwriteatt(unew, 'uEast', 'long_name', sprintf('%s, geo-rotated', ulname));
        ncwriteatt(unew, 'uEast', 'units', uunit);
    end
    
    Iv = ncinfo(vnew);
    if ~ismember('vNorth', {Iv.Variables.Name})
        nccreate(vnew, 'vNorth', ...
            'Dimensions', {'xi_rho', ...
                           'eta_rho', ...
                           's_rho', ...
                           'ocean_time'}, ...
            'Datatype', 'single');
        ncwriteatt(vnew, 'vNorth', 'long_name', sprintf('%s, geo-rotated', vlname));
        ncwriteatt(vnew, 'vNorth', 'units', vunit);
    end
    ...
    for ic = 1:nchunk
        ...
        u = ncread(ufile, 'u', scin{:});
        v = ncread(vfile, 'v', scin{:});
        ...
        ncwrite(unew, 'uEast',  uEast,  scout);
        ncwrite(vnew, 'vNorth', vNorth, scout);
